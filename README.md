...
- The Cloud sync panel shows the worker version from the worker's last answer, and warns when the worker is behind the userscript and needs redeploying. When the worker is ahead instead, an **⚠ Update script** button appears on screen and links to the latest release, because the userscript manager has not installed it yet.
